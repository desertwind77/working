...
int join(int fd, fifo_t *node_queue) {
	...
	/* Receive node_id */
	if(readn(fd, (char *)&node_id, sizeof(int))<0) {
		perror("Server:readn error\n");
		return -1;
	}
	...
	ptr = buffer;
	if(readn(fd, (char *)buffer, key_no*sizeof(int))<0) {
		perror("Server:readn error\n");
		return -1;
	}
...
int search(int fd, fifo_t *node_queue) {
	...
	// Receive number
	if(readn(fd, (char *)&key, sizeof(int))<0) {
		perror("Server:readn error\n");
		return -1;
	}
	...
	if( writen(fd, (char *)&result, sizeof(int))<0 ) {
	...
	if(result != -1) {
		...
		if( writen(fd, (char *)&size, sizeof(int)) < 0 ) {
			printf("Server: writen error\n");
			return(-1);
		}
		if( writen(fd, (char *)node->hostname, size) < 0 ) {
			printf("Server: writen error\n");
			return(-1);
		}
